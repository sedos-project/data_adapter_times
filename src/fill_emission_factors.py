...
SCEN_EMISSION_FILE_PATH = "output_data/Scen_emission_all_pow.xlsx"
...
def process_emission_factors(api_data, process_name, col_indices, ws):
    ...
    for api_col in api_data.columns:
        if api_col.startswith("ef_"):
            ...
            if "emi_co2_f" in cset_cn or "emi_co2_neg" in cset_cn:
                ...
                if (
                    ...
                    if remaining_string in global_emission_data.columns:
                        ...
                        api_value = (
                            global_emission_data[remaining_string].iloc[6]
                ...
                def load_desired_units_mapping(
                    file_path="config_data/units_mapping_pow.xlsx",
                ):
            ...
            attribute_value = (
                "ENV_ACT"
                if ("_p_" in cset_cn or "_proc_" in cset_cn)
                else (
                    "OUTPUT"
                    if "emi_co2_neg_air_" in cset_cn or "emi_co2_neg_imp" in cset_cn
                    else "FLO_EMIS"
                )
            )
...
def process_group_or_individual(process_name, ws, is_group=False):
    ...
    if is_group:
        ...
        for process, group_data in process_groups:
            # Skip processes starting with any prefix in ignore_prefixes
            if process.startswith(ignore_prefixes):
                continue
...
def main():
    ...
    global ignore_prefixes  # Declare global ignore prefixes
    ...
    pow_processes = [
        ...
        if not (
            process.startswith(ignore_prefixes)
            or (
                process.endswith("_ag")
                and not process.startswith(allowed_prefixes_for_ag)
            )
        )
    ]
