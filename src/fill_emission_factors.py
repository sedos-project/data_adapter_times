        ...
        if api_col.startswith("ef_"):
            ...
            if "emi_co2_f" in cset_cn or "emi_co2_neg" in cset_cn:
                ...
                def load_desired_units_mapping(
                    file_path="config_data/units_mapping_x2x.xlsx",
                ):
                ...
                if field_key in ef_units_mapping:
                    ...
                    # Extract a single numeric value if api_value is a list
                    value_to_convert = extract_single_value(api_value)
                    if value_to_convert is not None:
                        try:
                            converted_value, conversion_flag = convert_unit(
                                float(value_to_convert), source_unit, desired_unit
                            )
            ...
            attribute_value = (
                ...
                if ("_p_" in cset_cn or "_proc_" in cset_cn)
                    ...
                    "OUTPUT"
                    if "emi_co2_neg_" in cset_cn or "emi_co2_reusable" in cset_cn
                    else "FLO_EMIS"
                )
            )
...
def process_group_or_individual(process_name, ws, is_group=False):
    ...
    if not api_data.empty:
        ...
        if "version" in api_data.columns:
            api_data = api_data[api_data["version"] == "srd_point_draft"]
        else:
